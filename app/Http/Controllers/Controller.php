...
class Controller extends BaseController
{
    ...
    public function get_posts_query($post_type = 'noneposttype')
    {
        $posts = DB::table(POSTS_TABLE)
            ->leftJoin('postsmeta as thumbnails', function ($join) {
                $join->on('thumbnails.post_id', '=', 'posts.id')->where('thumbnails.meta_key', '=', 'thumbnails');
            })
            ->leftJoin('users', function ($join) {
                $join->on('users.id', '=', 'posts.post_author');
            })
            ->leftJoin('terms', function ($join) {
                $join->on('terms.id', '=', 'posts.term_id');
            })
            ->selectRaw(
    ...
    public function get_portfolio_query($post_type = 'noneposttype')
    {
        $posts = DB::table(POSTS_TABLE)
            ...
            ->leftJoin('postsmeta as portfolio_type', function ($join) {
                $join->on('portfolio_type.post_id', '=', 'posts.id')->where('portfolio_type.meta_key', '=', 'portfolio_type');
            })
            ->leftJoin('postsmeta as iframe_url', function ($join) {
                $join->on('iframe_url.post_id', '=', 'posts.id')->where('iframe_url.meta_key', '=', 'iframe_url');
            })
            ->leftJoin('postsmeta as video_url', function ($join) {
                $join->on('video_url.post_id', '=', 'posts.id')->where('video_url.meta_key', '=', 'video_url');
            })
            ->leftJoin('users', function ($join) {
                $join->on('users.id', '=', 'posts.post_author');
            })
