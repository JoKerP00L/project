...
class AdminAjaxController extends Controller
{
    ...
    public function adminajax(Request $request)
    {
        ...
        if($request->has('action') and $request->get('action') == 'megapanel_geticons')
        {
            ...
            $json_files['linearicons'] = ['title' => 'Linearicons', 'file' => public_path('dashboard/json/linearicons.json')];
            $json_files['peicon7stroke'] = ['title' => 'Pe Icon 7 Stroke', 'file' => public_path('dashboard/json/peicon7stroke.json')];

            if(has_option('fonticon', 'boxicons')){
                $json_files['boxicons'] = ['title' => 'Box icons', 'file' => public_path('dashboard/json/boxicons.json')];
            }

            if(has_option('fonticon', 'fontawesome')){
                $json_files['fontawesome'] = ['title' => 'Font Awesome', 'file' => public_path('dashboard/json/fontawesome.json')];
            }
